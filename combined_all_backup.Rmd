...
merged <- merge(merged, area_by_state, by.x = 'state', by.y = 'State.and.other.areas')
merged <- merged %>% 
  mutate(pop_density = total_population / Land.Area)
merged
```
...
masks  = mask %>%
  rename(
    ...
    merge(
   nyt_by_state, by.y = "state", by.x = "State"
  ) %>%
  mutate (
...
plot3 =  StateDeathsClosureMasks %>%
  ...
  geom_histogram() +
  xlab("Number of States") +
  ylab("Number of days Gym remained closed") +
...
plot4 = StateDeathsClosureMasks %>%
  ...
  geom_histogram() +
  xlab("Number of States") +
  ylab("Number of days movie remained closed") +
...
mean(StateDeathsClosureMasks$TotalMovieClosure)
```
...
plot(StateDeathsClosureMasks$pop_density, StateDeathsClosureMasks$deathsPerCapita, xlim=c(0,1000))
```

#### Model Building
```{r}
model1 <- lm(deathsPerCapita ~ maskMandateDaysAfterFirstMarch + did_implement_face_mandate + TotalRestClosure + NoOfDaysToSchoolClosing, data = StateDeathsClosureMasks)
model2 <- lm(deathsPerCapita ~  maskMandateDaysAfterFirstMarch + did_implement_face_mandate + TotalRestClosure + NoOfDaysToSchoolClosing + maskMandateTillStudyEnd + TotalGymClosure + TotalMovieClosure, data = StateDeathsClosureMasks)
model3 <- lm(deathsPerCapita ~ maskMandateDaysAfterFirstMarch + did_implement_face_mandate + TotalRestClosure + NoOfDaysToSchoolClosing + maskMandateTillStudyEnd + TotalGymClosure + TotalMovieClosure + pop_density + sixtyfive_plus_percentage, data = StateDeathsClosureMasks)
...
pairs(StateDeathsClosureMasks %>% 
        select (TotalMovieClosure, TotalRestClosure, TotalGymClosure, maskMandateDaysAfterFirstMarch))
...
```{r, echo = FALSE, message = FALSE, error = FALSE, warning = FALSE}
corrplot(cor(StateDeathsClosureMasks %>% select({model_2_variables})))
StateDeathsClosureMasks %>% 
  ggplot(aes(x = length, y = rate^2)) +
  geom_point() 
```
